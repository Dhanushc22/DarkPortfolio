...
function spawn() {
    ...
    return {
        ...
        tick(t) {
            ...
            let i = 0
            const visibilityRadius = innerWidth / 6; // Only show points within this distance
            pts.forEach((pt) => {
                const dx = pt.x - x,
                    dy = pt.y - y;
                const len = hypot(dx, dy);
                
                // Only process and paint points close to the spider
                if (len > visibilityRadius) {
                    pt.len = 0; // Fade out distant points
                    return;
                }
...
requestAnimationFrame(function anim(t) {
    ...
    if (w !== innerWidth) { w = innerWidth; canvas.width = innerWidth * dpr; }
    if (h !== innerHeight) { h = innerHeight; canvas.height = innerHeight * dpr; }
